    ...
    <header className="sticky top-0 z-50 border-b-[3px] border-line bg-bg">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden items-center gap-1 sm:flex" role="group" aria-label="accent color">
            {ACCENTS.map((a) => (
              <button
                key={a.id}
          ...
          <button
            ...
            className="hidden items-center gap-2 brutal-border bg-surface px-2.5 py-1.5 font-mono text-[11px] font-bold uppercase tracking-widest text-fg-dim hover:text-fg sm:flex"
          >
            <span>search</span>
            <kbd className="border-2 border-line px-1">K</kbd>
          </button>
          ...
          <a
            ...
            className="brutal-border bg-fg px-2.5 py-1.5 font-mono text-[11px] font-bold uppercase tracking-widest text-bg transition-transform hover:-translate-y-0.5"
          >
            src
          </a>
        </div>
      ...
      <nav className="flex items-center gap-1 overflow-x-auto border-t-[3px] border-line px-4 py-2 md:hidden">
        {NAV.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            scroll={false}
            className="whitespace-nowrap px-3 py-1 font-mono text-xs font-bold uppercase tracking-widest"
          >
            {item.label}
          </Link>
        ))}
